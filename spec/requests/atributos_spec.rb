require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/atributos", type: :request do
  
  # This should return the minimal set of attributes required to create a valid
  # Atributo. As you add validations to Atributo, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    skip("Add a hash of attributes valid for your model")
  }

  let(:invalid_attributes) {
    skip("Add a hash of attributes invalid for your model")
  }

  describe "GET /index" do
    it "renders a successful response" do
      Atributo.create! valid_attributes
      get atributos_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      atributo = Atributo.create! valid_attributes
      get atributo_url(atributo)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_atributo_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "renders a successful response" do
      atributo = Atributo.create! valid_attributes
      get edit_atributo_url(atributo)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Atributo" do
        expect {
          post atributos_url, params: { atributo: valid_attributes }
        }.to change(Atributo, :count).by(1)
      end

      it "redirects to the created atributo" do
        post atributos_url, params: { atributo: valid_attributes }
        expect(response).to redirect_to(atributo_url(Atributo.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Atributo" do
        expect {
          post atributos_url, params: { atributo: invalid_attributes }
        }.to change(Atributo, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post atributos_url, params: { atributo: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        skip("Add a hash of attributes valid for your model")
      }

      it "updates the requested atributo" do
        atributo = Atributo.create! valid_attributes
        patch atributo_url(atributo), params: { atributo: new_attributes }
        atributo.reload
        skip("Add assertions for updated state")
      end

      it "redirects to the atributo" do
        atributo = Atributo.create! valid_attributes
        patch atributo_url(atributo), params: { atributo: new_attributes }
        atributo.reload
        expect(response).to redirect_to(atributo_url(atributo))
      end
    end

    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        atributo = Atributo.create! valid_attributes
        patch atributo_url(atributo), params: { atributo: invalid_attributes }
        expect(response).to be_successful
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested atributo" do
      atributo = Atributo.create! valid_attributes
      expect {
        delete atributo_url(atributo)
      }.to change(Atributo, :count).by(-1)
    end

    it "redirects to the atributos list" do
      atributo = Atributo.create! valid_attributes
      delete atributo_url(atributo)
      expect(response).to redirect_to(atributos_url)
    end
  end
end
